...
const navLinks = [
    { href: '/dashboard/user', label: 'Complaints', roles: ['PUBLIC_USER', 'SUPER_ADMIN'] },
    { href: '/dashboard/agency', label: 'Agencies', roles: ['PUBLIC_USER', 'SUPER_ADMIN'] },
    { href: '/admin', label: 'Admin Portal', roles: ['SUPER_ADMIN'] },
    { href: '/dashboard/agency', label: 'Complaints (Agency)', roles: ['AGENCY_USER'] },
];
                ...
                <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                    ...
                    <div className={`${open ? '' : 'hidden'} w-full md:block md:w-auto`} id="navbar-default">
                        <ul className="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                            {navLinks.filter(link => !role || link.roles.includes(role)).map(link => (
                                <li key={link.href}>
